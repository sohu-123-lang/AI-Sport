...
class MultiModalModel(nn.Module):
    def __init__(self):
        ...
        # Transformer 处理 X1，批次在前
        self.transformer_encoder_layer = nn.TransformerEncoderLayer(d_model=HIDDEN_DIM, nhead=4, batch_first=True)
        ...
        self.transformer_gesture_layer = nn.TransformerEncoderLayer(d_model=HIDDEN_DIM, nhead=4, batch_first=True)
    ...
    def forward(self, x1, x2, x3, x4):
        ...
        x1_transformed = self.transformer_encoder(x1)  # (batch_size, seq_len, HIDDEN_DIM)
        ...
        x3_transformed = self.transformer_gesture(x3)  # (batch_size, seq_len, HIDDEN_DIM)
        ...
        # 融合所有输出
        combined = torch.cat((x1_out, x2_out, x3_out, x4_out), dim=1)  # 合并所有输出
...
def train_model(model, criterion, optimizer, train_loader, val_loader, scheduler=None):
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if (epoch + 1) % 1 == 0:  # 每100个epoch打印一次损失
            print(f'第 {epoch + 1} 轮, 损失: {loss.item():.4f}, 验证损失: {val_loss.item():.4f}')

        if (epoch + 1) % 2000 == 0:  # 每100个epoch打印一次损失
            torch.save(model.state_dict(), f'multi_modal_model_transformer-best-{epoch}.pth')
...
# 创建数据集
num_samples = 1656
X1 = torch.load('combined_X1.pt')  # 加载合并后的数据
X2 = torch.load('combined_X2.pt')
X3 = torch.load('combined_X3.pt')
X4 = torch.load('combined_X4.pt')
y = torch.load('combined_y.pt')

# 创建 TensorDataset 和 DataLoader
dataset = TensorDataset(X1, X2, X3, X4, y)
total_size = len(dataset)
train_size = int(0.8 * total_size)  # 90% 用于训练
val_size = int(0.1 * total_size)    # 5% 用于验证
...
# 保存模型
torch.save(model.state_dict(), 'multi_modal_model_transformer-best.pth')
